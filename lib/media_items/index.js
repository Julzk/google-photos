...
class MediaItems {
  ...
  async uploadBuffer(albumId, fileName, buffer, description, requestTimeout = 10000) {
    const url = `${constants.BASE_PATH}/:batchCreate`;
    const token = await this.transport.uploadBuffer(fileName, buffer, requestTimeout);
  ...
  updateDescription(mediaItemId, description) {
    return this.transport.patch(`${constants.BASE_PATH}/${mediaItemId}?updateMask=description`, {
      description,
    });
  }
}
